...
if ($action === 'create') {
    ...
    $docente_id = (int) $_POST['professor_id']; // frontend still sends professor_id
    ...
    // Check if another gestor already reserved this docente for conflicting dates/days/times
    ...
    // New: Check work schedule (Absolute block)
    $dow_names = [1 => 'Segunda-feira', 2 => 'Terça-feira', 3 => 'Quarta-feira', 4 => 'Quinta-feira', 5 => 'Sexta-feira', 6 => 'Sábado'];
    $dias_nomes_arr = array_map(function ($d) use ($dow_names) {
        return $dow_names[$d] ?? $d;
    }, $dias_arr);
    
    // Determine periodo (simple mapping or from POST)
    $periodo = $_POST['periodo'] ?? 'Integral'; 

    $workConflict = checkDocenteWorkSchedule($mysqli, $docente_id, $data_inicio, $data_fim, $dias_nomes_arr, $periodo, $hora_inicio, $hora_fim);
    if ($workConflict !== true) {
        echo json_encode(['ok' => false, 'error' => $workConflict]);
        exit;
    }
...
if ($action === 'delete') {
    ...
    $st = $mysqli->prepare("SELECT usuario_id FROM reservas WHERE id = ? AND status = 'APROVADA'");
...
if ($action === 'complete') {
    ...
    $st = $mysqli->prepare("SELECT * FROM reservas WHERE id = ? AND status = 'APROVADA'");
    $st->bind_param('i', $reserva_id);
    $st->execute();
    ...
    $st2 = $mysqli->prepare("UPDATE reservas SET status = 'CONCLUIDA' WHERE id = ?");
    $st2->bind_param('i', $reserva_id);
    $st2->execute();
...
// ── LIST RESERVAS ──
if ($action === 'list') {
    $status_filter = $_GET['status'] ?? 'APROVADA';
...
if ($action === 'check_professor') {
    ...
    $st = $mysqli->prepare("
        ...
        FROM reservas r
        JOIN usuario u ON r.usuario_id = u.id
        WHERE r.docente_id = ? AND r.status = 'APROVADA'
...
/**
 * Check if another gestor already reserved this docente for overlapping dates/days/times
 */
...
/**
 * Check if the docente already has agenda entries that conflict with the reservation
 */
